...
import { selectEdit, selectQuestionResponse, setDate, selectDate, setQuestionResponse } from "../question/QuestionSlice"
import { selectActiveStep, handleNext, handleBack, handleReset, changeToQuestion } from "./VerticalLinearStepperSlice"
import {
    selectQuestions,
...
import { selectPreviousAnswers } from "../pastAnswers/PastAnswersSlice"
...
export default function VerticalLinearStepper(props) {
    ...
    const obtainCurrentResponse = (step, activeStepChosen) => {
        if (questionsObjects.length > 0) {
            const foundQuestionObj = questionResponseItems.find(
                (item) => item.linkId == questionsObjects[activeStepChosen].linkId
            )
    ...
    const obtainPrevResponse = (step, activeStepChosen) => {
        if (questionsObjects.length > 0 && prevAnswers.length > 0) {
            const foundPrevObj = prevAnswers[0].answers.find(
                (item) => item.linkId == questionsObjects[activeStepChosen + step].linkId
            )
                            ...
                            {steps.map((label, index) => (
                                <Step key={label.prefix}>
                                    <StepLabel
                                        ...
                                        onClick={() => {
                                            activeStep > index
                                                ? dispatch(changeToQuestion(index)) && obtainPrevResponse(0, activeStep)
                                                : null
                                        }}
                                    >
            ...
            <div className={classes.sectionMobile}>
                {activeStep == steps.length ? (
                    <FormControl fullWidth margin="dense">
                        <Grid container direction="row" justify="space-between" align-items="flex-start" spacing={10}>
                            <Grid item xs={6}>
                                <Button
                                    size="small"
                                    onClick={() => {
